class NoSharedRoles {
  ...
  async run() {
    try {
      const functionsWithIdenticalRoles = this.stackFunctions.reduce((acc, current) => this.hasRoleBeenEncountered(current) ? [...acc, current] : acc, []);

      this.failingResources = functionsWithIdenticalRoles.map(lambda => ({arn: lambda.FunctionArn, Role: lambda.Role}));
